...
typedef enum SYM_ENUM_e {
    ...
    S_ENVBLOCK      =  0x113d,  // Environment block split off from S_COMPILE2 - same info as original EnC data (which we've emitted regardless of whether EnC is enabled since VS 2005
...
typedef struct ENVBLOCKSYM {
    ...
    struct {
        unsigned char  fEC              : 1;    // Compiled for Edit & Continue
        unsigned char  pad              : 7;    // reserved, must be 0
    } flags;
    unsigned char   rgsz[1];    // Sequence of zero-terminated strings with EnC info.
} ENVBLOCKSYM;
